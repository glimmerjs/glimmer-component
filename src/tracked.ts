...
import { dict, Dict, Stack, Option } from '@glimmer/util';

class TrackStack {
  private stack = new Stack<Tracker>();

  push() {
    this.stack.push(new Tracker());
  }

  pop(): Tracker {
    return this.stack.pop() as Tracker;
  }

  get current(): Option<Tracker> {
    return this.stack.current;
  }

  add(tag: Tag) {
    let tracker = this.current;
    if (tracker) tracker.add(tag);
  }
}
...
function installTrackedProperty(target: any, key: Key) {
  ...
  Object.defineProperty(target, key, {
    ...
    get() {
      STACK.add(metaFor(this).updatableTagFor(key));
